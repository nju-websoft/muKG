...
class IPTransE(BasicModel):
    ...
    def ref_sim_mat(self):
        ref1_embeddings = F.normalize(self.ent_embeds(to_tensor(self.ref_entities1, self.device)), 2, -1).cpu().detach().numpy()
        ref2_embeddings = F.normalize(self.ent_embeds(to_tensor(self.ref_entities2, self.device)), 2, -1).cpu().detach().numpy()
    ...
    def generate_transE_loss(self, data):
        ...
        """
        phs = F.normalize(self.ent_embeds(phs), 2, -1)
        prs = F.normalize(self.rel_embeds(prs), 2, -1)
        pts = F.normalize(self.ent_embeds(pts), 2, -1)
        nhs = F.normalize(self.ent_embeds(nhs), 2, -1)
        nrs = F.normalize(self.rel_embeds(nrs), 2, -1)
        nts = F.normalize(self.ent_embeds(nts), 2, -1)
        prx = F.normalize(self.rel_embeds(prx), 2, -1)
        pry = F.normalize(self.rel_embeds(pry), 2, -1)
        pr = F.normalize(self.rel_embeds(pr), 2, -1)
        nrx = F.normalize(self.rel_embeds(nrx), 2, -1)
        nry = F.normalize(self.rel_embeds(nry), 2, -1)
