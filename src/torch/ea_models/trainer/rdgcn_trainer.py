...
from src.py.util.util import early_stop, to_var, generate_out_folder
...
class rdgcn_trainer(align_model_trainer):
    ...
    def init(self, args, kgs):
        ...
        self.model = Layer(self.args, self.kgs, self.local_name_vectors)
        self.model.init()
        self.model.to(self.device)
        # self.output, self.loss = self.gcn_model.build()
        self.optimizer = get_optimizer_torch('Adam', self.model, self.args.learning_rate)
    ...
    def training1(self):
        ...
        for i in range(1, self.args.max_epoch + 1):
            ...
            batch_loss = self.model.build({"neg_left": to_var(neg_left, self.device),
    ...
    def test(self, save=True):
        embedding = self.model.get_output()
