...
def n_gram_compositional_func(character_vectors, value_lens, batch_size, embed_size):
    ...
    pos_c_e_lstm = calculate_ngram_weight(value_lens, pos_c_e_in_lstm, batch_size, embed_size)
    return pos_c_e_lstm


def calculate_ngram_weight(value_len, stacked_tensor, batch_size, embed_size):
    # stacked_tensor = tf.stack(stacked_tensor, axis=1)
    stacked_tensor = tf.reverse(stacked_tensor, [1])
    # index = tf.constant(len(stacked_tensor))
    index = value_len
...
class AttrE(BasicModel):
    ...
    def _define_embed_graph(self):
        ...
        with tf.name_scope('triple_lookup'):
            ...
            pvs = add_compositional_func(pvs)
            nvs = add_compositional_func(nvs)
        ...
        with tf.name_scope('triple_loss'):
            ...
            self.triple_loss_ce = get_loss_func_tf(pes, pas, pvs, nes, nas, nvs, self.args)
            self.triple_optimizer_ce = generate_optimizer_tf(self.triple_loss_ce, self.args.learning_rate,
                                                          opt=self.args.optimizer)

            cos_sim = tf.reduce_sum(tf.multiply(ents_se, ents_ce), 1, keep_dims=True)
            self.joint_loss = tf.reduce_sum(1 - cos_sim)
            self.optimizer_joint = generate_optimizer_tf(self.joint_loss, self.args.learning_rate, opt=self.args.optimizer)
