...
from src.py.util.util import early_stop
from src.tf.ea_models.basic_model import BasicModel
from src.tf.ea_models.gcn_align import ones, glorot, zeros
...
class Layer:
    ...
    def add_dual_att_layer(self, inlayer, inlayer2, adj):
        a = tf.expand_dims(inlayer2, 0)
        in_fts = tf.layers.conv1d(a, self.dim, 1)
...
class RDGCN(BasicModel):
    ...
    def save(self):
        embedding = self.sess.run(self.output)
        read.save_embeddings(self.out_folder, self.kgs, embedding, None, None, mapping_mat=None)
