...
class TransE_ET(BasicModel):
    ...
    def __init__(self, kgs, args):
        ...
        # 初始化关系翻译向量空间
        self.rel_embeddings = init_embeddings([1, self.dim], 'rel_embeddings',
                                              'xavier', False, dtype=tf.float32)

        self.type_embeddings = init_embeddings([3851, self.dim], 'type_embeddings',
                                              'xavier', False, dtype=tf.float32)
    def calc(self, h, r, t):
        if self.p_norm == 'L1':
    ...
    def get_embeddings(self, h, r, t, mode='entity'):
        ...
        proj_t = tf.expand_dims(tf.nn.embedding_lookup(self.type_embeddings, t), 1)  # shape: (b_size, 1, emb_dim)
        candidates = tf.reshape(self.type_embeddings, [1, 3851, self.dim])
    ...
    def call(self, data):
        ...
        t = tf.nn.embedding_lookup(self.type_embeddings, batch_t)
